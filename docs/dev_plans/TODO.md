...
## Signal Delivery to Active RuntimeExecutor Threads — ✅ RESOLVED

All items implemented. See [`signal-delivery-to-active-executor.md`](signal-delivery-to-active-executor.md).

| Component | What was built |
|-----------|---------------|
| `SignalChannels` | `Arc<Mutex<HashMap<u32, mpsc::Sender<Signal>>>>` — replaces per-agent Unix sockets; registered at spawn, deregistered at exit in `IpcExecutorFactory` |
| `tokio::select!` in `run_with_client` | Races `run_turn_streaming` against `signal_rx.recv()` — signals arrive during LLM call, not just between turns (`dispatch_manager.rs:1158`) |
| `CancellationToken` | Created per LLM turn; cancelled immediately on `SIGKILL`/`SIGSTOP`/`SIGPAUSE` arrival |
| `handle_signal_during_llm` | Distinct handler for in-flight signals — sets `killed`/`paused` atomics, flushes VFS status (`dispatch_manager.rs:976`) |
| `handle_signal_between_turns` | Existing handler for inter-turn signals (`dispatch_manager.rs:899`) |
| Per-agent socket files | Deleted — `SignalDelivery` and `agent_socket.rs` removed |
