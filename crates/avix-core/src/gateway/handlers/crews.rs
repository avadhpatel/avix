...
pub async fn handle(cmd: ValidatedCmd, ctx: &HandlerCtx) -> AtpReply {
    ...
    match op {
        "list" | "get" | "create" | "update" | "delete" | "join" | "leave" => {
            ipc_forward(
                &id,
                &format!("kernel/crews/{op}"),
                cmd.cmd.body,
                ctx.ipc.as_ref(),
            )
            .await
...
mod tests {
    ...
    fn make_cmd(op: &str) -> ValidatedCmd {
        ValidatedCmd {
            cmd: crate::gateway::atp::frame::AtpCmd {
                ...
                id: "c-1".into(),
                token: "tok".into(),
                domain: AtpDomain::Crews,
                op: op.into(),
                body: json!({}),
            },
            caller_identity: "alice".into(),
            caller_role: Role::User,
            caller_session_id: "sess-crews".into(),
        }
    }
    ...
    #[tokio::test]
    async fn list_translates_to_kernel_crews_list() {
        let ctx = make_ctx("kernel/crews/list", json!({"crews": []})).await;
        let reply = handle(make_cmd("list"), &ctx).await;
        assert!(reply.ok);
    }

    #[tokio::test]
    async fn join_translates_to_kernel_crews_join() {
        let ctx = make_ctx("kernel/crews/join", json!({})).await;
        let reply = handle(make_cmd("join"), &ctx).await;
        assert!(reply.ok);
    }
    ...
    async fn unknown_op_returns_eparse() {
        ...
        let reply = handle(make_cmd("kick"), &ctx).await;
        assert!(!reply.ok);
