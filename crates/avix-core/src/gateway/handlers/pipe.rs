...
mod tests {
    ...
    fn make_cmd(op: &str) -> ValidatedCmd {
        ValidatedCmd {
            cmd: crate::gateway::atp::frame::AtpCmd {
                ...
                id: "pp-1".into(),
                token: "tok".into(),
                domain: AtpDomain::Pipe,
                op: op.into(),
                body: json!({}),
            },
            caller_identity: "alice".into(),
            caller_role: Role::User,
            caller_session_id: "sess-pipe".into(),
        }
    }
    ...
    async fn pipe_open_translates_to_kernel_pipe_open() {
        let ctx = make_ctx("kernel/pipe/open", json!({"pipe_id": "pipe-abc"})).await;
        let reply = handle(make_cmd("open"), &ctx).await;
        assert!(reply.ok);
    }

    #[tokio::test]
    async fn pipe_close_translates_to_kernel_pipe_close() {
        let ctx = make_ctx("kernel/pipe/close", json!({})).await;
        let reply = handle(make_cmd("close"), &ctx).await;
        assert!(reply.ok);
    }
    ...
    async fn unknown_op_returns_eparse() {
        ...
        let reply = handle(make_cmd("flush"), &ctx).await;
        assert!(!reply.ok);
