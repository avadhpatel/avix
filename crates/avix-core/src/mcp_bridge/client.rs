...
impl From<McpClientError> for crate::error::AvixError {
    fn from(e: McpClientError) -> Self {
        match e {
            McpClientError::Io(s) => crate::error::AvixError::McpUnreachable(s),
            McpClientError::Protocol(s) => crate::error::AvixError::McpProtocol(s),
            McpClientError::Timeout => {
                crate::error::AvixError::McpUnreachable("request timed out".into())
            }
            McpClientError::ServerGone => {
                crate::error::AvixError::McpUnreachable("server process gone".into())
            }
        }
    }
}
