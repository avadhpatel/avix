...
impl InvocationStore {
    ...
    pub async fn write_conversation(
        ...
        for (role, content) in messages {
            ...
            lines.push_str(&serde_json::to_string(&line).map_err(|e| AvixError::ConfigParse(e.to_string()))?);
            lines.push('\n');
        }
        let rel = format!(
...
mod tests {
    ...
    async fn list_for_user_filters_by_username() {
        let store = open_store().await;
        store.create(&make_record("a", "alice", "bot")).await.unwrap();
        store.create(&make_record("b", "bob", "bot")).await.unwrap();
        store.create(&make_record("c", "alice", "coder")).await.unwrap();
    ...
    async fn list_for_agent_filters_by_agent() {
        let store = open_store().await;
        store.create(&make_record("a", "alice", "researcher")).await.unwrap();
        store.create(&make_record("b", "alice", "coder")).await.unwrap();
    ...
    async fn finalize_unknown_id_is_idempotent() {
        let store = open_store().await;
        let result = store
            .finalize("does-not-exist", InvocationStatus::Completed, Utc::now(), 0, 0, None)
            .await;
        assert!(result.is_ok());
    }
    ...
    async fn two_invocations_for_same_agent_do_not_collide() {
        let store = open_store().await;
        store.create(&make_record("x1", "alice", "researcher")).await.unwrap();
        store.create(&make_record("x2", "alice", "researcher")).await.unwrap();
        let result = store.list_for_agent("alice", "researcher").await.unwrap();
